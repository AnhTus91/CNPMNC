...
namespace CNPMNC_Giao.Controllers
{
    public class CartController : Controller
    {
        ...
        //            {
        //                // Thêm tiêu đề đơn hàng
        //                document.Add(new Paragraph($"Đơn Hàng #{donHang.MaDonHang}").SetFontSize(18).SetBold());
        //                document.Add(new Paragraph($"Khách Hàng: {donHang.TenNguoiNhan}"));
        //                document.Add(new Paragraph($"Ngày Đặt Hàng: {donHang.NgayDatHang.ToShortDateString()}"));
        //                document.Add(new Paragraph($"Địa Chỉ Nhận Hàng: {donHang.DiaChiNguoiNhan}"));
        //                document.Add(new Paragraph($"Số Điện Thoại: {donHang.SDTNguoiNhan}"));
        //                document.Add(new Paragraph("\nChi Tiết Đơn Hàng:").SetFontSize(16).SetBold());
        ...
        //                    if (chiTiet != null)
        //                    {
        //                        var sanPham = db.SanPham.Find(chiTiet.MaSanPham); // Lấy thông tin sản phẩm

        //                        if (sanPham != null)
        //                        {
        //                            table.AddCell(sanPham.TenSanPham); // Tên sản phẩm
        //                            table.AddCell(chiTiet.Soluong.ToString()); // Số lượng
        //                            table.AddCell(chiTiet.DonGia.ToString("C")); // Đơn giá
        //                            table.AddCell((chiTiet.DonGia * chiTiet.Soluong).ToString("C")); // Thành tiền
        //                        }
        //                        else
        //                        {
        //                            // Nếu sản phẩm không tồn tại, thêm thông báo vào bảng
        //                            table.AddCell("Sản phẩm không tồn tại");
        ...
        //                document.Add(new Paragraph($"\nTổng Tiền: {donHang.TongTien.ToString("C")}").SetFontSize(14).SetBold());
        //            }
        //        }

        //        // Trả về file PDF cho người dùng tải về
        //        return File(stream.ToArray(), "application/pdf", "don_hang.pdf");
        //    }
        //}
