...
namespace CNPMNC_Giao.Areas.Admin.Controllers
{
    public class SanPhamsController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "MaSanPham,TenSanPham,GiaTienMoi,GiaTienCu,MoTa,AnhSP,MaVatLieu,MaDanhMuc,NgayTao,MaNhaCungCap")] SanPham sanPham, HttpPostedFileBase AnhSanPham)
        {
            ...
            var existingProduct = db.SanPham.FirstOrDefault(sp => sp.TenSanPham == sanPham.TenSanPham);
            if (existingProduct != null)
            {
                // Trả về lỗi nếu sản phẩm đã tồn tại
                ModelState.AddModelError("TenSanPham", "Sản phẩm này đã tồn tại trong cửa hàng.");
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                _FileName = idImage + "." + AnhSanPham.FileName.Substring(index + 1);
...
SanPham sanPham, HttpPostedFileBase AnhSanPham)
        {
            if (ModelState.IsValid)
            {
                var existingProduct = db.SanPham.AsNoTracking().FirstOrDefault(sp => sp.MaSanPham == sanPham.MaSanPham);
                if (existingProduct == null)
                {
                    return HttpNotFound();
                }
